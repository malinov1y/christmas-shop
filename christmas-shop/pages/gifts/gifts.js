...
//giftsCategory
const categoryButtons = document.querySelectorAll('.gifts-container__tab');
const cardsContainer = document.querySelector('.gifts-cards');
let currentCategory = 'all';

function createCards(filteredCards) {
    cardsContainer.innerHTML = '';
    filteredCards.forEach(gift => {
        const card = document.createElement('div');
        card.classList.add('gifts-card');
        ...
        card.innerHTML = `
            <img class="gifts-card__img" src="${image}" alt="${gift.category}">
            <div class="gifts-card__text">
                <h4 class="gifts-card__text-h4 ${categoryClass}">${gift.category}</h4>
                <h3 class="gifts-card__text-h3">${gift.name}</h3>
            </div>
        `;
...
function filterCards(category) {
    if (category === 'all') {
        ...
        const filteredCards = gifts.filter(gift => gift.category.toLowerCase() === category);
...
function updateActiveButton(selectedCategory) {
    categoryButtons.forEach(button => {
        if (button.textContent.toLowerCase() === selectedCategory || (selectedCategory === 'all' && button.textContent.toLowerCase() === 'all')) {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  cards.forEach(card => {
    card.addEventListener('click', function() {
      const categoryElement = card.querySelector('.gifts-card__text-h4');
      const category = categoryElement.textContent;
      const categoryClass = Array.from(categoryElement.classList).find(className => className !== 'gifts-card__text-h4');
      const name = card.querySelector('.gifts-card__text-h3').textContent;
      const imgSrc = card.querySelector('.gifts-card__img').src;
